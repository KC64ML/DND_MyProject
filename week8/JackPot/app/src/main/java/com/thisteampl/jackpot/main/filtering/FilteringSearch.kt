...
class FilteringSearch : AppCompatActivity() {
    ...
    // API 작성 DB에 넘김
    // 사용자 정보가 디자인일 때 디자인, 개발자일 때 개발자
    private fun userjobcheck() {


//        userapi?.getUserPosition(recruitmentproject)
    ...
    private fun searchfindbtn() {
        ...
        filtersearch_applyfilter_button.setOnClickListener {
            ...
            if(page == 1){
                ...
                Log.d("tag","관심분야 ")
                Log.d("tag","사이즈 : ${memberfind_projectfieldbtn.size}")
                for(num in 0..memberfind_projectfieldbtn.size-1){
                    Log.d("", "결과 : ${memberfind_projectfieldbtn[num]}")
                }
        ...
        for (i in 0 until filtersearch_page2_field_linearlayout.childCount) {
            filtersearch_page2_field_linearlayout[i].setOnClickListener {
                if(membercheckbool[i] == false){
                    ...
                    memberfind_projectfieldbtn.add(memberintentionbtn[i]?.text.toString())
                }else{
                    ...
                    memberfind_projectfieldbtn.remove(memberintentionbtn[i]?.text.toString())
                }
            }
        ...
        for (i in 0 until filtersearch_page2_field2_linearlayout.childCount) {
            filtersearch_page2_field2_linearlayout[i].setOnClickListener {
                if(membercheckbool2[i] == false){
                    ...
                    memberfind_projectfieldbtn.add(memberintentionbtn2[i]?.text.toString())
                }else{
                    ...
                    memberfind_projectfieldbtn.remove(memberintentionbtn2[i]?.text.toString())
                }
            }
